...
export class ToolBridge {
    ...
    registerTransport(type, handler) {
    ...
    applyTemplate(template, data, args) {
        return this._renderTemplate(template, { ...(data || {}), ...args });
    }
    ...
    // ── Private ──
    async _applyTransform(code, response, args) {
...
export class RestTransport {
    async execute(tool, args, signal) {
        if (!tool.url)
            throw new Error('REST tool requires a URL');
    ...
    _interpolate(template, args) {
        return template.replace(/\{\{(\w+)\}\}/g, (_, key) => String(args[key] ?? ''));
    }
    _interpolateValues(obj, args) {
        const result = {};
...
export class FunctionTransport {
    async execute(tool, args, _signal) {
        if (!tool.execute)
            throw new Error('Function tool requires execute code');
...
export class BrowserApiTransport {
    async execute(tool, args, _signal) {
        const api = tool.api;
        if (!api)
            throw new Error('browser-api tool requires an api name');
        switch (api) {
            case 'clipboard':
...
export class MCPTransport {
    async execute(tool, args, signal) {
        if (!tool.server?.url)
            throw new Error('MCP tool requires server URL');
        if (!tool.toolName)
            throw new Error('MCP tool requires toolName');
