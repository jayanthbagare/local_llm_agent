...
const functionWithTransformTool = {
    type: 'function',
...
describe('ToolBridge', () => {
    ...
    describe('execute', () => {
        ...
        it('returns error for unknown tool type', async () => {
            const result = await bridge.execute({ type: 'unknown' }, {});
    ...
    describe('applyTemplate', () => {
        it('renders a mustache-style template', () => {
            const result = bridge.applyTemplate('Results for {{query}}: {{count}} items', { count: 5 }, { query: 'test' });
            expect(result).toBe('Results for test: 5 items');
        });
        it('handles missing values', () => {
            const result = bridge.applyTemplate('{{a}} and {{missing}}', {}, {});
    ...
    describe('custom transports', () => {
        it('supports registering custom transport', async () => {
            ...
            const result = await bridge.execute({ type: 'custom' }, { x: 1 });
            expect(result.success).toBe(true);
...
describe('FunctionTransport', () => {
    ...
    it('executes simple math', async () => {
        const result = await transport.execute({ type: 'function', execute: 'return params.x * 2;' }, { x: 21 });
        expect(result).toBe(42);
    });
    it('has access to Math functions', async () => {
        const result = await transport.execute({ type: 'function', execute: 'return Math.sqrt(params.n);' }, { n: 16 });
        expect(result).toBe(4);
    });
    it('has access to JSON', async () => {
        const result = await transport.execute({ type: 'function', execute: 'return JSON.stringify({ a: params.val });' }, { val: 42 });
        expect(result).toBe('{"a":42}');
    });
    it('handles errors', async () => {
        await expect(transport.execute({ type: 'function', execute: 'throw new Error("fail");' }, {})).rejects.toThrow('fail');
    });
    it('rejects missing execute code', async () => {
        await expect(transport.execute({ type: 'function' }, {})).rejects.toThrow('requires execute code');
    });
});
...
describe('RestTransport', () => {
    ...
    it('interpolates URL parameters', async () => {
        ...
        expect(fetch).toHaveBeenCalledWith('https://api.example.com/data?q=hello', expect.objectContaining({ method: 'GET' }));
    ...
    it('handles HTTP errors', async () => {
        ...
        await expect(transport.execute(restTool, { query: 'test' })).rejects.toThrow('404');
    });
    it('rejects missing URL', async () => {
        await expect(transport.execute({ type: 'rest', method: 'GET' }, {})).rejects.toThrow('requires a URL');
    });
});
// ── MCP Transport Tests ──
describe('MCPTransport', () => {
    ...
    it('rejects missing server URL', async () => {
        await expect(transport.execute({ type: 'mcp', toolName: 'test' }, {})).rejects.toThrow('requires server URL');
    });
    it('rejects missing toolName', async () => {
        await expect(transport.execute({ type: 'mcp', server: { transport: 'sse', url: 'http://localhost' } }, {})).rejects.toThrow('requires toolName');
    });
});
...
describe('BrowserApiTransport', () => {
    ...
    it('rejects unknown API', async () => {
        await expect(transport.execute({ type: 'browser-api', api: 'nonexistent' }, {})).rejects.toThrow('Unknown browser API');
    });
    it('rejects missing api name', async () => {
        await expect(transport.execute({ type: 'browser-api' }, {})).rejects.toThrow('requires an api name');
    });
    it('returns calendar mock data', async () => {
        const result = await transport.execute({ type: 'browser-api', api: 'calendar' }, { action: 'list' });
