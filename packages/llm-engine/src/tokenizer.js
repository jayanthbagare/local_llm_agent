 ...
 *   {{m.role}}  {{m.content}}
 */
export function applyChatTemplate(template, messages, addGenerationPrompt = true) {
    ...
    // Handle {% if gen %}...{% endif %}
    template = template.replace(/\{%\s*if\s+gen\s*%\}([\s\S]*?)\{%\s*endif\s*%\}/g, (_, content) => addGenerationPrompt ? content : '');
    return template.trim();
}
function renderMessageBlock(body, m) {
