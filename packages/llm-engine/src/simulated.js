...
export class SimulatedEngine {
    ...
    getModelInfo() { return this._info; }
    async generate(options) {
        const tokens = [];
        let text = '';
        for await (const t of this.generateStream(options)) {
            tokens.push(t);
            text += t.text;
        }
    ...
    _generate(options) {
        ...
        if (!userMsg)
            return 'I am ready to help. What would you like to know?';
    ...
    _getLastUserMessage(options) {
        if (options.prompt)
            return options.prompt;
        if (options.messages) {
            for (let i = options.messages.length - 1; i >= 0; i--) {
                if (options.messages[i].role === 'user')
                    return options.messages[i].content;
            }
        }
        return null;
    }
    _countPromptTokens(options) {
        if (options.messages)
...
export function createSimulatedEngine() {
